...
public final class ClipboardHistoryPanel: NSObject, DockablePanel {
    ...
    /// Whether the pasteboard is currently being polled.
    ...
    /// Pasteboard types by which an application asks not to be recorded.
    ///
    /// This is the nspasteboard.org convention, and password managers rely on
    /// it: 1Password, Keychain Access and the rest mark a copied secret
    /// `ConcealedType`. Ignoring it means a clipboard history quietly collects
    /// the user's passwords and shows them in a panel, which is the single
    ...
    func capturePasteboardIfChanged() {
        ...
        guard Self.mayRecord(pasteboard) else { return }
        guard let text = pasteboard.string(forType: .string), !text.isEmpty else { return }
        record(text)
    }

    /// Whether this pasteboard's owner allows its contents to be kept.
    static func mayRecord(_ pasteboard: NSPasteboard) -> Bool {
        let available = pasteboard.types ?? []
        return !available.contains { excludedTypes.contains($0) }
    }
